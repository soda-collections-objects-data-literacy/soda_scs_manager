...
class SodaScsSnapshotHelpers {
  ...
  /**
   * The triplestore component actions.
   *
   * @var \Drupal\soda_scs_manager\ComponentActions\SodaScsComponentActionsInterface
   */
   ...
   * @param \Drupal\soda_scs_manager\Entity\SodaScsSnapshotInterface $snapshot
   *   The snapshot.
   * @param bool $pseudoSnapshot
   *   Whether is a pseudo snapshot.
   *
   * @return \Drupal\soda_scs_manager\ValueObject\SodaScsResult
   *   The result of the request.
   */
  public function restoreFromSnapshot(SodaScsSnapshotInterface $snapshot, bool $pseudoSnapshot): SodaScsResult {
    ...
    $checksumValidation = $this->validateSnapshotChecksum($snapshot, $filePath);
    if (!$checksumValidation['success']) {
      return SodaScsResult::failure(
    ...
    $unpackResult = $this->unpackSnapshotToTempDirectory($filePath, $tempDir);
    ...
    $manifestData = $this->parseAndValidateManifest($manifestPath);
   ...
   *   Array with success status and restoration data.
   */
  private function restoreComponentsFromManifest(array $manifestData, ?SodaScsSnapshotInterface $snapshot = NULL, string $tempDir): array {
    $restorationResults = [];
    $errors = [];
    ...
    foreach ($manifestData['mapping'] as $componentMapping) {
      ...
      try {
        ...
        if (!$snapshot) {
          ...
          $snapshot = $this->createPseudoSnapshotForComponent($component, $dumpFile, $tempDir);
        }
